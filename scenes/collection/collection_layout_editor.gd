...
@onready var left_menu: Panel = $LeftMenu
@onready var hero_roster: Panel = $HeroRoster
@onready var pose_idle: Panel = $Pose_IDLE
@onready var pose_hit: Panel = $Pose_HIT
@onready var pose_cast: Panel = $Pose_CAST
@onready var pose_attack: Panel = $Pose_ATTACK
@onready var skins_menu: Panel = $SkinsMenu
...
func _setup_hero_roster() -> void:
	...
	hero_roster.add_child(title)
	
	roster_container = HBoxContainer.new()
	roster_container.position = Vector2(20, 30)
	roster_container.add_theme_constant_override("separation", 10)
	hero_roster.add_child(roster_container)
	...
	for hero_id in HeroDatabase.heroes.keys():
		...
		roster_container.add_child(thumbnail)
...
func _setup_skins_container() -> void:
	# Clear existing label
	for child in skins_menu.get_children():
		child.queue_free()
	...
	title.position = Vector2(70, 10)
	title.add_theme_font_size_override("font_size", 16)
	skins_menu.add_child(title)
	
	skins_container = VBoxContainer.new()
	skins_container.position = Vector2(20, 50)
	skins_container.add_theme_constant_override("separation", 10)
	skins_menu.add_child(skins_container)
...
func _setup_pose_panel(panel: Panel, pose_name: String, sprite_path: String, role_color: Color) -> void:
	...
	if ResourceLoader.exists(sprite_path):
		...
		var tex_size = sprite.texture.get_size() if sprite.texture else Vector2(100, 100)
		sprite.position = Vector2((panel_size.x - tex_size.x) / 2, 10)
	sprite.name = "Sprite"
	...
	label.position = Vector2(0, panel.size.y - 30)
	label.custom_minimum_size = Vector2(panel.size.x, 25)
...
func _update_tab_visuals() -> void:
	...
	hero_roster.visible = show_heroes or show_cards
	
	# Pose panels only for Heroes tab
	pose_idle.visible = show_heroes
	pose_hit.visible = show_heroes
	pose_cast.visible = show_heroes
	pose_attack.visible = show_heroes
	
	# Skins menu for Heroes, Card info for Cards
	skins_menu.visible = show_heroes
...
func _toggle_card_info_panel(show: bool) -> void:
	if show:
		...
		if card_info_panel and is_instance_valid(card_info_panel):
			card_info_panel.visible = false

func _create_card_info_panel() -> void:
	# Create panel at same position as skins_menu
	card_info_panel = Panel.new()
	card_info_panel.position = skins_menu.position
	card_info_panel.custom_minimum_size = skins_menu.size
	card_info_panel.size = skins_menu.size
	...
	style.corner_radius_bottom_left = 5
	style.corner_radius_bottom_right = 5
	card_info_panel.add_theme_stylebox_override("panel", style)
	
	var title = Label.new()
	title.text = "CARD INFO"
	title.position = Vector2(50, 10)
	title.add_theme_font_size_override("font_size", 16)
	card_info_panel.add_child(title)
	
	var info_label = Label.new()
	info_label.name = "InfoLabel"
	info_label.text = "Click a card to view details"
	info_label.position = Vector2(10, 50)
	info_label.custom_minimum_size = Vector2(skins_menu.size.x - 20, skins_menu.size.y - 60)
	info_label.autowrap_mode = TextServer.AUTOWRAP_WORD
	info_label.add_theme_font_size_override("font_size", 12)
	info_label.add_theme_color_override("font_color", Color(0.8, 0.8, 0.8))
	card_info_panel.add_child(info_label)
...
func _display_cards_in_pose_panels() -> void:
	...
	# Get positions from pose panels
	var positions = [
		pose_idle.position,
		pose_hit.position,
		pose_cast.position,
		pose_attack.position
	]
	var panel_size = pose_idle.size
	
	# Create card panels at same positions as pose panels
	for i in range(min(hero_cards.size(), 4)):
		var card_id = hero_cards[i]
		var card_data = CardDatabase.get_card(card_id)
		if card_data.is_empty():
			continue
		
		var card_panel = _create_card_panel(card_data, role_color, positions[i], panel_size)
...
func _create_card_panel(card_data: Dictionary, role_color: Color, pos: Vector2, size: Vector2) -> Panel:
	...
	if not art_path.is_empty() and ResourceLoader.exists(art_path):
		...
		var template_id = HeroDatabase.get_hero_template(selected_hero_id)
		...
		skill_art.position = Vector2(art_region.get("x", 8), art_region.get("y", 18))
		skill_art.custom_minimum_size = Vector2(art_region.get("width", 144), art_region.get("height", 88))
		skill_art.size = Vector2(art_region.get("width", 144), art_region.get("height", 88))
		...
		if ResourceLoader.exists(frame_path):
			...
			template_frame.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
	...
	overlay.position = Vector2(3, size.y - 80)
	overlay.custom_minimum_size = Vector2(size.x - 6, 77)
	overlay.size = Vector2(size.x - 6, 77)
	overlay.color = Color(0, 0, 0, 0.7)
	panel.add_child(overlay)
	
	# Card name (inside card, at bottom)
	var name_label = Label.new()
	name_label.text = card_data.get("name", "Unknown")
	name_label.position = Vector2(5, size.y - 75)
	name_label.custom_minimum_size = Vector2(size.x - 10, 22)
	name_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	name_label.add_theme_font_size_override("font_size", 13)
	name_label.add_theme_color_override("font_color", Color(1, 1, 1))
	panel.add_child(name_label)
	
	# Card cost (inside card)
	var cost = card_data.get("cost", 0)
	var cost_text = str(cost) if cost >= 0 else "X"
	var cost_label = Label.new()
	cost_label.text = "Cost: " + cost_text + "  |  " + card_data.get("type", "").capitalize()
	cost_label.position = Vector2(5, size.y - 55)
	cost_label.custom_minimum_size = Vector2(size.x - 10, 18)
	cost_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	cost_label.add_theme_font_size_override("font_size", 10)
	cost_label.add_theme_color_override("font_color", Color(0.7, 0.85, 1.0))
	panel.add_child(cost_label)
	
	# Card description (inside card, truncated)
	var desc_label = Label.new()
	var desc = card_data.get("description", "")
	if desc.length() > 60:
		desc = desc.substr(0, 57) + "..."
	desc_label.text = desc
	desc_label.position = Vector2(5, size.y - 35)
	desc_label.custom_minimum_size = Vector2(size.x - 10, 30)
	...
	# Clickable button to show full info
	var button = Button.new()
	button.flat = true
	...
	button.position = Vector2(0, 0)
	button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	button.pressed.connect(_on_card_clicked.bind(card_data))
